...
```{r}
cur_comp <- sort(unique(partition_list))
...
num_cells_in_partition == dim(cds)[2] ## if cur_comp is 1, all cells are in paritition
```
...
```{r}
find_nearest_vertex <- function(data_matrix, target_points, block_size=50000, process_targets_in_blocks=TRUE){
  closest_vertex = c()
  if (process_targets_in_blocks == FALSE){
    ...
    for (i in 1:num_blocks){
      ...
      closest_vertex_for_block <- apply(distances_Z_to_Y, 1,  which.min)
...
kmean_res <- stats::kmeans(t(X_subset), centers=centers, iter.max = 100)
nearest_center <- find_nearest_vertex(t(kmean_res$centers), X_subset, process_targets_in_blocks=TRUE)
length(nearest_center) # nearest target point to kmeans center
```
...
t = seq(0, 2 *pi, length.out = ncol(blah)) 
medioids = matrix(c(sin(t), cos(t)), nrow=2, byrow=T) * 5
colnames(medioids) <- colnames(blah)
...
prune_tree <- function(stree_ori, stree_loop_closure, minimal_branch_len = 10) {
  if (ncol(stree_ori) < minimal_branch_len) return(stree_loop_closure);
  ...
  stree_loop_closure <- igraph::graph_from_adjacency_matrix(stree_loop_closure, mode = 'undirected', weight = NULL)
  ...
  if(nrow(valid_edges) > 0) {
    vertex_top_keep <- as.character(unlist(apply(valid_edges, 1,
                                function(x) {
  ...
  mst_traversal <- igraph::graph.dfs(stree_ori, root = root_cell, neimode = "all", unreachable=FALSE, father=TRUE)
...
```{r}
rge_res_W <- res$ddrtree_res_W
rge_res_Z <- res$ddrtree_res_Z
rge_res_Y <- res$ddrtree_res_Y
cds <- res$cds
dp_mst <- res$dp_mst
...
project2MST <- function(cds, Projection_Method, orthogonal_proj_tip = FALSE, verbose, reduction_method, rge_res_Y){
  ...
  if(!is.function(Projection_Method)) {
    ...
    for(i in 1:length(closest_vertex)) { # This loop is going to be slow
      neighbors <- names(igraph::neighborhood(
        dp_mst,
...
prune_graph <- TRUE
minimal_branch_len <- 10
multi_tree_DDRTree_res = rge(cds, blah)
```
